...
sub expr { debug( "Trying expr for $in\n"); 
    ( start_expr()||return undef);
    my $tail =  collect_expr("expression list");
    end_expr(); 
    debug( Dumper ("Expression returned: ", $tail));return $tail; 
}
...
sub dump { my $self = shift; return join( "", "( ", $self->dumpnobr, ") ") }
sub dumpnobr { my $self = shift; return join ("", map {$_->dump} @{$self->value}) }
sub dumpbracefree { my $self = shift; return join("",  join("", map {$_->dumpbracefree} @{$self->value}) ) }
...
package Closure;
sub dump { my $self = shift; join("", "( ", $self->dumpnobr , ") ") }
sub dumpnobr { my $self = shift; join(" ", "lambda", $self->{args}->dump, $self->{body}->dump) }
